  ...
  useEffect(() => {
    ...
    async function loadFormData() {
      try {
        ...
        const data = await getVideoFormData();
        ...
        if (isMounted) {
          setError("Failed to load video form data.");
        }
      } finally {
        if (isMounted) {
  ...
  async function handleUpload(data: UploadVideoFormData) {
    try {
      ...
      if (data.method === "url") {
        ...
        await uploadLessonVideo(data);

        setUploadStatus("processing");
        navigate(`/admin/videos?lesson=${data.lesson_id}`);
        return;
      }
      ...
      if (data.method === "device") {
        ...
        const validationError = validateVideoFile(data.video_file);
        ...
        const directUpload = await createMuxDirectUpload(data.lesson_id);
        ...
        setSuccessMessage(
          "Video uploaded successfully. Mux is processing it now. The lesson will be connected after webhook verification."
        );
      }
    } catch {
      setUploadStatus("failed");
      setError("Failed to upload video. You can retry the upload.");
    } finally {
    ...
    <section className="space-y-6">
      <div>
        <Link
          to={
            lessonIdFromUrl
              ? `/admin/videos?lesson=${lessonIdFromUrl}`
              : "/admin/videos"
          }
        ...
        <p className="mt-1 text-sm text-slate-500">
          Upload by URL or directly from device using Mux Direct Upload.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-slate-700">
            Upload Status:{" "}
            <span className="capitalize text-sky-600">{uploadStatus}</span>
          </p>
